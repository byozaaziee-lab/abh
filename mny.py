...
async def callback_handler(c, q: CallbackQuery):
    ...
    if data.startswith("gift_transfer_"):
        ...
        if balance < 25:
            ...
            # Hapus waiting input dan hanya jawab "callback" tanpa balasan
            waiting_input.pop(uid, None)
            await q.answer("❌ Gagal.", show_alert=True)  # hanya alert kecil tanpa pesan detail
            return
        # Transfer gift
        ...
        if success:
            ...
            # Gagal karena error lain, beri tahu user (tanpa mention owner)
            await q.message.reply(msg)
        waiting_input.pop(uid, None)
        await q.answer()
        return
