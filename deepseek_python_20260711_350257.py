...
# ==================== PERBAIKAN SET 2FA (pakai method bawaan) ====================
async def set_2fa_password(app, new_password):
    try:
        await app.set_2fa_password(new_password)
...
async def callback_handler(c, q: CallbackQuery):
    ...
    if data.startswith("gift_transfer_"):
        ...
        if balance < 25:
            # Kirim DM ke OWNER saja, user tidak dapat balasan apapun
            ...
            waiting_input.pop(uid, None)
            await q.answer("❌ Gagal.", show_alert=True)  # hanya alert tanpa pesan detail
            return
        # Transfer gift
...
if __name__ == "__main__":
    ...
    print("   • 📡 OTP & PESAN")
    print("   • 🔐 SET/UBAH 2FA (fix menggunakan method bawaan)")
